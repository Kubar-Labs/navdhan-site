...
import { intakePausedResponse, jsonResponse } from "./errors";
import { isApplyIntakeAvailable } from "@/src/lib/apply/intake-mode";
 ...
 * the approved GCP application-gateway URL in a deployed environment.
 *
...
function applyBackendBaseUrl(): string {
  ...
  if (parsed.username || parsed.password || parsed.search || parsed.hash) {
    console.error(
      "APPLY_BACKEND_BASE_URL cannot contain credentials, a query, or a fragment.",
    );
    throw new Error("APPLY_BACKEND_BASE_URL contains unsupported URL components");
  }
  if (parsed.pathname !== "/" && parsed.pathname !== "") {
    console.error("APPLY_BACKEND_BASE_URL must be an origin without a path.");
    throw new Error("APPLY_BACKEND_BASE_URL must not contain a path");
  }
  if (
    process.env.NODE_ENV === "production" &&
    (parsed.hostname === "navdhan.app" || parsed.hostname === "www.navdhan.app")
  ) {
    console.error("APPLY_BACKEND_BASE_URL must not point back to the public website.");
    throw new Error("APPLY_BACKEND_BASE_URL must not use the public website origin");
  }
...
export async function requestApplyBackend(
  ...
  if (!isApplyIntakeAvailable()) throw new Error("Application intake is paused");
...
export function backendUnavailableResponse(): Response {
  if (!isApplyIntakeAvailable()) return intakePausedResponse();
