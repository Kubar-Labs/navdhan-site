...
describe("apply backend service boundary", () => {
  ...
  it("fails closed on backend redirects without following them", async () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("APPLY_INTAKE_MODE", "live");
  ...
  it.each([
    ...
    "https://gateway.example?destination=elsewhere",
    "https://gateway.example#fragment",
    "https://navdhan.app",
    "https://www.navdhan.app",
  ])("rejects an unsafe production backend origin: %s", async (baseUrl) => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("APPLY_INTAKE_MODE", "live");
    vi.stubEnv("APPLY_BACKEND_BASE_URL", baseUrl);
    vi.stubEnv("APPLY_BACKEND_SERVICE_TOKEN", SERVICE_TOKEN);
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("fails closed before contacting the backend when production intake is paused", async () => {
    vi.stubEnv("NODE_ENV", "production");
    vi.stubEnv("APPLY_INTAKE_MODE", "paused");
    vi.stubEnv("APPLY_BACKEND_BASE_URL", "https://backend.example");
    vi.stubEnv("APPLY_BACKEND_SERVICE_TOKEN", SERVICE_TOKEN);
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
