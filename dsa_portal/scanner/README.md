...
hash-locked Python graph and Poppler's `pdfinfo`, refreshes official signatures
during the build and again before accepting traffic, and runs `freshclam`
hourly as the non-root `clamav` user. `clamscan` rejects signature databases
older than two days and files that exceed its inspection limits. After a clean
malware verdict, `pdfinfo` classifies PDFs that cannot be parsed or opened due
to document permissions as terminal `unreadable`; parser runtime failures stay
retryable.
