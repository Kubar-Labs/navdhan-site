...
class ScannerPrimitiveTests(unittest.TestCase):
        ...
        with tempfile.NamedTemporaryFile(suffix=".pdf") as document:
            ...
            for return_code, expected in ((0, "clean"), (1, "infected")):
                with self.subTest(return_code=return_code), patch(
                    ...
                    side_effect=[
                        subprocess.CompletedProcess([], return_code),
                        *(
                    ...
                    if return_code == 0:
                        self.assertEqual(["pdfinfo", str(path)], run.call_args.args[0])
                    else:
    ...
    def test_pdf_parser_distinguishes_unreadable_bytes_from_runtime_failure(
        self,
    ) -> None:
        with tempfile.NamedTemporaryFile(suffix=".pdf") as document:
            path = Path(document.name)
            for return_code in (1, 3):
                with self.subTest(return_code=return_code), patch(
                    "scanner_app.subprocess.run",
                    side_effect=[
                        subprocess.CompletedProcess([], 0),
                    ...
                    self.assertEqual("unreadable", _scan_file(path))

            for return_code in (2, 99, -9):
                with self.subTest(return_code=return_code), patch(
                    "scanner_app.subprocess.run",
                    side_effect=[
                        subprocess.CompletedProcess([], 0),
            ...
            with patch(
                ...
                side_effect=[
                    subprocess.CompletedProcess([], 0),
                    subprocess.TimeoutExpired("pdfinfo", 30),
                ],
            ):
                with self.assertRaises(ScannerUnavailableError):
                    _scan_file(path)
