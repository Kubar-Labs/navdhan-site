...
def _scan_file(path: Path) -> str:
    ...
    if completed.returncode != 0:
        ...
        # and engine errors. Retrying preserves the quarantine object; accepting
        # a terminal unreadable verdict here would permanently discard a file
        # that was never reliably scanned.
        raise ScannerUnavailableError("ClamAV could not produce a verdict")

    # The upload boundary checks the PDF signature and trailer, while Poppler
    # supplies the independent parser check needed to distinguish a genuinely
    # unreadable document from a scanner outage. Run this only after ClamAV so
    # a known-infected file never reaches the additional parser.
    try:
        ...
        raise ScannerUnavailableError("PDF parser did not complete") from error
    if pdf_info.returncode == 0:
        return "clean"
    if pdf_info.returncode in (1, 3):
        # pdfinfo documents 1 as a PDF-open/parse error and 3 as a PDF
        # permissions error (for example, an encrypted document without the
        # required password). Both are properties of these bytes and will not
        # improve when Eventarc retries the same immutable generation.
        return "unreadable"
    # Output-file failures, internal parser failures, crashes, and unexpected
    # exit statuses describe the scanner runtime rather than the document.
    raise ScannerUnavailableError("PDF parser could not produce a verdict")
