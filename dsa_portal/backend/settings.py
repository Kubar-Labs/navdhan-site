...
DEFAULT_APP_ENV = "dev"
DEFAULT_VERIFICATION_PROVIDER_MODE = "disabled"
...
class Settings:
    ...
    verification_provider_mode: str
...
def load_settings() -> Settings:
    ...
    verification_provider_mode = (
        os.getenv("VERIFICATION_PROVIDER_MODE") or DEFAULT_VERIFICATION_PROVIDER_MODE
    ).strip().lower()
    if verification_provider_mode != "disabled":
        raise ConfigurationError(
            "VERIFICATION_PROVIDER_MODE must remain 'disabled' until an approved "
            "provider adapter is implemented and passes staging acceptance"
        )
    ...
    return Settings(
        ...
        verification_provider_mode=verification_provider_mode,
