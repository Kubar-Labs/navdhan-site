...
class TestDatabaseGuardTests(unittest.TestCase):
    ...
    def test_guard_accepts_the_configured_dedicated_test_database(self) -> None:
        db_test_support.TEST_PG_DSN = "postgresql://postgres@127.0.0.1:55432/navdhan_test"
        self.assertEqual("navdhan_test", guard_test_database_name())

    def test_guard_refuses_when_configured_database_name_is_the_dev_database(self) -> None:
        db_test_support.TEST_PG_DSN = DEV_DATABASE_URL.replace("+asyncpg", "")

        with self.assertRaisesRegex(UnsafeTestDatabaseError, "development database"):
            guard_test_database_name()

    def test_guard_refuses_a_database_name_without_the_test_suffix(self) -> None:
        db_test_support.TEST_PG_DSN = "postgresql://postgres@127.0.0.1:55432/navdhan_staging"
    ...
    def test_guard_refuses_a_live_connection_to_the_dev_database(self) -> None:
        ...
        async def probe() -> None:
            connection = await asyncpg.connect(DEV_DATABASE_URL.replace("+asyncpg", ""))
            try:
    ...
    def test_guard_refuses_when_live_connection_disagrees_with_configured_name(self) -> None:
