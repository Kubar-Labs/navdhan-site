...
> **Release gate:** every new PDF is quarantined. A release may receive
> production traffic only after the environment-specific scanner, dispatcher,
> and IAM path are verified and the quarantine-to-clean flow passes staging.
...
Cloud Run candidates, Eventarc triggers, environment-specific IAM, monitoring,
and reconciliation remain external infrastructure and must be provisioned and
exercised in staging before each release is approved.
...
## 3. Staging rehearsal and conditional rebuild

Before release, compare the staging migration/seed ledger and runtime-visible
seed counts with the exact candidate commit. If staging contains the
superseded 60-row checklist, the current seed is upsert-only and cannot remove
those stale rows, so re-seeding is forbidden; rebuild it from the current
branch. If the ledger checksums and expected `1 / 3 / 36 / 5 / 23` seed counts
already match, do not rebuild a correct staging database.
