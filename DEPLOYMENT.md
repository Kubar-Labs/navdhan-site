...
| Application gateway | External HTTPS Application Load Balancer + Cloud Armor + serverless NEG |
...
Deploy a private Cloud Run Job using the backend image and override its command
to `python -m maintenance.retention --marketplace-id <uuid> --execute`. Create a
separate scheduled invocation for each marketplace; the job deliberately does
not bypass tenant RLS or discover tenants with an administrative role. Review a
dry run first by omitting `--execute`.

The policy is 30 days since the last update for never-submitted drafts and 60
calendar months since submission for submitted records. Prove both boundaries,
shared-person/borrower preservation, generation-matched GCS deletion,
idempotent retries, audit emission, and storage-failure rollback in staging.
The scheduler identity may invoke only the retention job. The job identity may
connect only as the runtime database role and delete only from its environment's
document bucket. Do not schedule production until legal approves these periods
and a restore rehearsal has been recorded.
...
Identify the candidate revision without moving normal service traffic. Test it
through the staging application gateway or an operator-authenticated private
diagnostic path; never expose its tagged URL publicly:
...
The backend Cloud Run service must use `internal-and-cloud-load-balancing`
ingress and receive public-origin traffic only through the approved external
HTTPS Application Load Balancer/serverless NEG. Cloud Armor restricts methods,
request size, and abusive traffic at the gateway. Direct `run.app` ingress is
not a supported path. FastAPI remains a second authentication boundary: it
compares `x-navdhan-service-token` against `APPLY_SERVICE_TOKEN` in constant
time before every `/api/apply` route. Only `/health` is unauthenticated, and it
is exposed through the gateway solely for health checking. Do not promote a
revision if direct-ingress or negative-token tests fail.

Provider verification is a separate private service boundary, not code running
inside the public Worker. The shipped backend registry supports only
`VERIFICATION_PROVIDER_MODE=disabled`. Do not add a verifier route, provider
credentials, or egress allow-list exception until an approved provider-hosted
consent contract and signed-callback flow pass staging. Tax-portal and banking
passwords must never enter NavDhan systems.
...
- `APPLY_BACKEND_BASE_URL`: the canonical HTTPS GCP application-gateway origin
  in front of the private Cloud Run service; and
...
The browser talks to `navdhan.app`, never to the GCP gateway or Cloud Run.
CORS is defense in depth only and is not the access-control boundary.
