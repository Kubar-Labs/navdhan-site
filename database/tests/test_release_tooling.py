...
class DatabaseReleaseToolingTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.retired_frontend_path = PROJECT_ROOT / "dsa_portal/ci/cloudbuild-frontend.yaml"
        cls.legacy_scripts_path = PROJECT_ROOT / "dsa_portal/scripts"
    ...
    def test_legacy_and_direct_deployment_paths_are_absent(self) -> None:
        self.assertFalse(self.retired_frontend_path.exists())
        if self.legacy_scripts_path.exists():
            self.assertEqual([], list(self.legacy_scripts_path.glob("deploy-*.ps1")))
